...
forms.find(form => form.id === 'job').fields.push(text('City', 'city'), text('State / Province', 'state'), text('Country', 'country'), text('University attended', 'university'), text('Degree', 'degree'), text('Cover letter', 'coverLetter', 'file', { accept: '.pdf,.docx,.txt' }));
forms.find(form => form.id === 'speaker').fields.push(text('Presentation slides', 'slides', 'file', { accept: '.pdf,.ppt,.pptx' }), text('Speaker headshot', 'headshot', 'file', { accept: 'image/*' }));
forms.find(form => form.id === 'freelance').fields.push(text('Work sample', 'workSample', 'file', { accept: '.pdf,.zip,.png,.jpg' }), text('Certification', 'certificate', 'file', { accept: '.pdf,.txt' }));
forms.find(form => form.id === 'company').fields.push(text('Company logo', 'logo', 'file', { accept: 'image/*' }));
experiences.job.sections[12] = ['Location & education', 'Details to help us understand your background.'];
experiences.speaker.sections[10] = ['Supporting files', 'Share your slides and a photo for the programme.'];
experiences.freelance.sections[10] = ['Supporting documents', 'Attach a sample of your work and relevant qualifications.'];
...
for (const config of forms) {
  ...
  for (const [index, field] of config.fields.entries()) {
    ...
    if (field.type === 'file') { input.accept = field.accept || '.pdf,.doc,.docx'; label.classList.add('upload'); }
