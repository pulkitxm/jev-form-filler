...
Embedded application forms, including Ashby forms inside company careers pages, are scanned and filled in their own document. Open the **Application** tab first. On first use, **Fill Details** requests access to the embedded site, such as `jobs.ashbyhq.com`. Once allowed, popup filling and the right-click shortcut work on that embedded form. Without access, the page status directs you to the popup. Undo and saved-file attachment target the original document; navigating the frame invalidates old answers. Hidden frames are excluded.
...
Standard visible text, email, URL, telephone, number, textarea, single-select, and standard file-upload controls are supported. Passwords, hidden fields, identified financial/identity credentials, disabled fields, and read-only controls are excluded. Checkboxes, radio buttons, custom upload widgets and other custom controls, closed shadow roots, and built-in browser pages are not supported. A scan handles up to 60 fields per document. Changes to a field's value, identity, or page after scanning cause that field to be skipped. Nothing submits the form automatically.
...
**Fill Details** matches the upload label and accepted file types against the saved file metadata. File contents are never sent to TypeSafe. Ambiguous versions appear in the popup for explicit selection. Existing attachments are preserved. Undo clears an attachment only if it has not been replaced manually. Undo cannot retract a file that a website has already uploaded. Some websites upload immediately when a file is attached, even before form submission. Standard file inputs are supported, including the clipped native resume input inside Ashby’s drop zone. Widgets without a usable native file input and inputs hidden with `display:none` still need manual handling.
