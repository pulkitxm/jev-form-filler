...
manifest.host_permissions.push('https://www.linkedin.com/*', 'https://portfolio.test/*', 'https://forms.test/*', 'https://api.github.com/*');
...
try {
  ...
  const tabsBefore = await worker.evaluate(async () => (await chrome.tabs.query({})).length);
  ...
  let linkedinBlocked = true;
  let linkedinFetches = 0;
  await context.route('https://www.linkedin.com/**', route => {
    if (route.request().resourceType() !== 'document') linkedinFetches++;
    return route.fulfill({ contentType: 'text/html', body: linkedinBlocked ? '<title>Sign in to LinkedIn</title><h1>Sign in</h1>' : profileHtml });
  });
  await app.locator('[data-view="sources"]').click();
  await app.locator('[data-connector="linkedin"]').click();
  await app.locator('#source-url').fill('https://www.linkedin.com/in/alex-example/');
  const profileTabPromise = context.waitForEvent('page');
  await app.getByRole('button', { name: 'Open profile in browser', exact: true }).click();
  const profileTab = await profileTabPromise;
  await profileTab.goto('https://www.linkedin.com/in/alex-example/');
  await app.locator('#import-connected').click();
  await app.getByText('This is a sign-in or browser-check page, not profile content. Open the actual profile and import the loaded page.', { exact: true }).waitFor();
  assert.equal(await app.locator('#pending-profile').isVisible(), true);
  linkedinBlocked = false;
  await profileTab.reload();
  await app.locator('#import-connected').click();
  await app.getByText('Open page imported. Build your profile to review its details.', { exact: true }).waitFor();
  assert.equal(await app.locator('#pending-profile').isHidden(), true);
  ...
  await socialPopup.getByRole('button', { name: 'Import this profile', exact: true }).waitFor();
  await socialPopup.close();
  console.log('PASS: social connection opens a rendered profile tab, preserves pending import through sign-in, imports loaded content, and exposes the popup shortcut.');
  await app.locator('[data-view="settings"]').click();
