...
export function extractDocument(doc, url) {
  ...
  add(host.endsWith('linkedin.com') ? 'linkedin' : ['x.com', 'twitter.com', 'www.x.com', 'www.twitter.com'].includes(host) ? 'twitter' : host === 'github.com' ? 'github' : 'website', url);
...
export async function importGithub(value, options = {}) {
  ...
  return { url: user.html_url, title: user.name || username, candidates, importedAt: new Date().toISOString() };
}
export function capturePage() {
  const clone = document.documentElement.cloneNode(true);
  for (const node of clone.querySelectorAll('script:not([type="application/ld+json"]),style,noscript,nav,footer,form,input,textarea,select,button,[hidden],[aria-hidden="true"]')) node.remove();
  return { url: location.href, html: clone.outerHTML.slice(0, 1_500_000) };
}
